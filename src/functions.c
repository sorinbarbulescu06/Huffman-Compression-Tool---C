...
t_lit make_sorted_array(int *freq) //NULL = aloc err, else = ok
{
    int i;
...
//decompress.c
int string_found(int argc, char *argv[], char *s) //1 = found, 0 = not found
{
    int i;
    if (argc == 2) { //printing everything
        return 1;
    }
    for (i = 2; i < argc; ++i) {
...
void print_or_not(FILE *fin, FILE *fout, unsigned char *buf, int *buf_count, t_tree_node root, int *ok, t_tree_node original_root)
{
    ...
    while (*ok == 1) {
        if (current->rt == NULL && current->lt == NULL) { //leaf case
            if (current->ch == NO_CHAR - 1) {
                *ok = 0;
                break;
            } else {
                if (fout != NULL) {
        ...
        if (*buf_count == 0) {
            *buf_count = 8;
            ...
            if (fread(buf, sizeof(unsigned char), 1, fin) != 1) {
                break; 
            }
            *buf = reverse_bits(*buf);
        }
